...
# include path for core noobdawn API
INCLUDEPATH += $$_PRO_FILE_PWD_/../noobdawn/api/replay
...
win32 {
	...
	RC_INCLUDEPATH = $$_PRO_FILE_PWD_/../noobdawn/api/replay
	RC_FILE = Resources/qnoobdawn.rc
	...
	swig.commands = $$_PRO_FILE_PWD_/3rdparty/swig/swig.exe -v -Wextra -Werror -O -interface ${QMAKE_FILE_BASE} -c++ -python -modern -modernargs -enumclass -fastunpack -py3 -builtin -I$$_PRO_FILE_PWD_ -I$$_PRO_FILE_PWD_/../noobdawn/api/replay -outdir . -o ${QMAKE_FILE_BASE}_python.cxx ${QMAKE_FILE_IN}
	...
	SWIGSOURCES += Code/pynoobdawn/noobdawn.i
	SWIGSOURCES += Code/pynoobdawn/qnoobdawn.i
	...
	LIBS += $$DESTDIR/noobdawn.lib

	# Link against the version library
	LIBS += $$DESTDIR/version.lib

	QMAKE_CXXFLAGS_WARN_ON -= -w34100 
	DEFINES += NOOBDAWN_PLATFORM_WIN32

} else {
	isEmpty(CMAKE_DIR) {
		error("When run from outside CMake, please set the Build Environment Variable CMAKE_DIR to point to your CMake build root. In Qt Creator add CMAKE_DIR=/path/to/noobdawn/build under 'Additional arguments' in the qmake Build Step. If running qmake directly, add CMAKE_DIR=/path/to/noobdawn/build/ to the command line.")
	}
	...
	include($$CMAKE_DIR/qnoobdawn/qnoobdawn_cmake.pri)
	...
	SOURCES += $$CMAKE_DIR/qnoobdawn/noobdawn_python.cxx
	SOURCES += $$CMAKE_DIR/qnoobdawn/qnoobdawn_python.cxx
	...
	macx: {
		...
		DEFINES += NOOBDAWN_PLATFORM_POSIX NOOBDAWN_PLATFORM_APPLE
		ICON = $$OSX_ICONFILE

		# add qrc file with qt.conf
		RESOURCES += Resources/qtconf.qrc
		
		librd.files = $$files($$DESTDIR/../lib/libnoobdawn.dylib)
		...
		QMAKE_POST_LINK += sh $$_PRO_FILE_PWD_/../util/set_plist_version.sh $${NOOBDAWN_VERSION}.0 $${INFO_PLIST_PATH}
	} else {
		QT += x11extras
		DEFINES += NOOBDAWN_PLATFORM_POSIX NOOBDAWN_PLATFORM_LINUX NOOBDAWN_WINDOWING_XLIB NOOBDAWN_WINDOWING_XCB
...
SOURCES += Code/qnoobdawn.cpp \
    ...
    Code/Resources.cpp \
    Code/RGPInterop.cpp \
    Code/pynoobdawn/PythonContext.cpp \
...
HEADERS += Code/CaptureContext.h \
    ...
    Code/Resources.h \
    Code/RGPInterop.h \
    Code/pynoobdawn/PythonContext.h \
    Code/pynoobdawn/pyconversion.h \
    Code/pynoobdawn/interface_check.h \
