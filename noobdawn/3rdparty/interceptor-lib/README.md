...
Minor modifications have been made to include it in NoobDawn's build system.
...
To use this in NoobDawn you must set `-DUSE_INTERCEPTOR_LIB=On -DLLVM_DIR=/path/to/llvm` pointing to a compatible build of LLVM 4.0 for the target you are building. Below are instructions on how to build that compatible LLVM.
...
Then you can set `-DUSE_INTERCEPTOR_LIB=On -DLLVM_DIR=/path/to/llvm/install_armXX/lib/cmake/llvm` when building noobdawn and you should see a line saying that LLVM is being used to compile interceptor-lib!
