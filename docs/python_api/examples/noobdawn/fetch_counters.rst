...
The first thing we do is enumerate a list of counters that the implementation supports using :py:meth:`~noobdawn.ReplayController.EnumerateCounters`. A few of these counters values are statically known - see :py:class:`~noobdawn.GPUCounter`. If you know which counter you want ahead of time you can continue straight away by calling :py:meth:`~noobdawn.ReplayController.FetchCounters` with a list of counters to sample from, or :py:meth:`~noobdawn.ReplayController.DescribeCounter` to obtain a :py:class:`~noobdawn.CounterDescription` of the counter itself:
