...
# Import noobdawn if not already imported (e.g. in the UI)
if 'noobdawn' not in sys.modules and '_noobdawn' not in sys.modules:
	import noobdawn

# Alias noobdawn for legibility
rd = noobdawn
...
if 'pynoobdawn' in globals():
	raise RuntimeError("This sample should not be run within the NoobDawn UI")
else:
	if len(sys.argv) <= 1:
		print('Usage: python3 {} filename.nbd'.format(sys.argv[0]))
		sys.exit(0)
