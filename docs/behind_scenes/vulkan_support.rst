...
This page documents the support of Vulkan in NoobDawn. This gives an overview of what NoobDawn is capable of, and primarily lists information that is relevant. You might also be interested in the :doc:`full list of features <../getting_started/features>`.

Vulkan capture
--------------

NoobDawn's Vulkan support uses the sophisticated layering mechanism built into the Vulkan API. This means that once NoobDawn has registered its capture layers, there is no need for invasive process hooking - we can simply launch the target process with an environment variable set.
...
Usually this will happen for you if you install NoobDawn with a proper installer, or update from a previous version. However if you are compiling NoobDawn yourself or running a loose unzipped build, the capture panel will show a warning indicating that the layers need to be registered. Just click this and the registration will be applied.
...
NoobDawn has support for Vulkan version 1.4, as well as a number of extensions, but it contains some caveats. In addition, not all replay features are currently supported, but this is being actively worked on.

* NoobDawn assumes that even if multiple GPUs are present, that only one VkDevice will be created and used.
* NoobDawn captures will in general not be portable between different systems, only currently supporting capture and replay on the same or similar enough machines.

For more information about NoobDawn's raytracing support see :doc:`raytracing`. Note that raytracing support is limited on nvidia drivers due to missing driver functionality.
