...
NoobDawn supports python extensions using the :doc:`existing python API <../python_api/index>`. These extensions can be configured to be persistent and load whenever the NoobDawn UI does, allowing users to extend the program with custom functionality.

Example extensions can be found at the `community contributed repository <https://github.com/baldurk/noobdawn-contrib>`_. More information on writing an extension is available at :doc:`../python_api/ui_extensions`.

Creating extensions
-------------------

Extensions are simply python modules located in the user's NoobDawn config folder, with a json manifest. The config folder varies by platform, on Windows it's ``%APPDATA%\qnoobdawn\extensions`` and on linux it's ``~/.local/share/qnoobdawn/extensions``. Each extension is a python module subfolder under this root. You can nest subfolders, e.g. ``extensions/foo/bar/first`` would be the extension ``foo.bar.first``, and treated independently from ``extensions/foo/bar/second``.
...
The ``minimum_noobdawn`` field controls whether the extension can be enabled on a given NoobDawn version, and allows you to block incompatible versions of NoobDawn.
...
Without this json manifest the extension will not be enumerated by NoobDawn and displayed to users.
...
.. code:: python
    ...
    def register(version, pynoobdawn):
        # version is the NoobDawn Major.Minor version as a string, such as "1.2"
        # pynoobdawn is the CaptureContext handle, the same as the global available in the python shell
...
Extensions use the full :doc:`python API <../python_api/index>` with all of the power that comes with it. There are interfaces geared specifically towards extensions in :doc:`the qnoobdawn module <../python_api/qnoobdawn/extensions>` which allow registering callbacks for menu items.
