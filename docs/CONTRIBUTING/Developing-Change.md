...
* Limit STL use to a minimum. Allowed uses are `std::map`, `std::function`, `std::set`, algorithms like `std::sort` and `std::lower_bound`, and type-traits type checks like `std::is_same`. Use `nbdarray` instead of `std::vector` and `nbdstr` instead of `std::string`.
...
There are always plenty of things to do, if you'd like to chip in! Check out the [issues](https://github.com/baldurk/noobdawn/issues) for outstanding feature requests.

If you have a change you'd like to see make it into mainline, create a fork of noobdawn, make your changes to a branch, and open a pull request on github. You can look around for instructions on that - it's pretty simple.
